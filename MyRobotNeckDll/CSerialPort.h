#ifndef _C_SERIAL_PORT_H_
#define _C_SERIAL_PORT_H_

#ifdef C_SERIAL_PORT_EXPORTS
#define C_SERIAL_PORT_API __declspec(dllexport)
#else
#define C_SERIAL_PORT_API __declspec(dllimport)
#endif

#include <Windows.h>

class C_SERIAL_PORT_API CSerialPort
{
public:
	CSerialPort();
	virtual ~CSerialPort();
	
private:
	HANDLE			m_hComm;
	DCB				m_dcb;
	COMMTIMEOUTS		m_CommTimeouts;
	bool				m_bPortReady;
	bool				m_bWriteRC;
	bool				m_bReadRC;
	DWORD			m_iBytesWritten;
	DWORD			m_iBytesRead;
	DWORD			m_dwBytesRead;

public:
	void ClosePort();
	bool ReadByte(BYTE& resp);
	bool ReadByte(BYTE* &resp, UINT size);
	bool WriteByte(BYTE data);
	bool OpenPort(const char* portname);
	bool SetCommunicationTimeouts(DWORD ReadIntervalTimeout,
		DWORD ReadTotalTimeoutMultiplier, DWORD ReadTotalTimeoutConstant,
		DWORD WriteTotalTimeoutMultiplier, DWORD WriteTotalTimeoutConstant);
	bool Configure(DWORD BaudRate, BYTE ByteSize, DWORD fParity,
						BYTE Parity, BYTE StopBits);
};
#endif